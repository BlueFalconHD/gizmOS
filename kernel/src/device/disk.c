...
g_bool disk_init(disk_t *d)
{
  if (!d || !d->vblk) {
    ...
    printf("disk_init: failed to create disk. d: %{type: ptr}, d->vblk: %{type: ptr}\n", PRINT_FLAG_BOTH, d, d->vblk);
    #endif
    return false;
  }
  if (!d->vblk->vdev || !d->vblk->vdev->is_initialized) {
    #if DISK_DRIVER_DEBUG
    printf("disk_init: failed to initialize disk. d->vblk->vdev: %{type: ptr}, d->vblk->vdev->is_initialized: %{type: bool}\n", PRINT_FLAG_BOTH, d->vblk->vdev, d->vblk->vdev->is_initialized); 
    #endif
  }
