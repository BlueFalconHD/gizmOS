...
static size_t ring_bytes_avail(uint16_t size) { return sizeof(struct virtq_avail) + sizeof(uint16_t) * size; }
static size_t ring_bytes_used(uint16_t size)  { return sizeof(struct virtq_used)  + sizeof(struct virtq_used_elem) * size; }
...
int virtq_create(virtio_device_t *dev, uint16_t qidx, uint16_t requested_size, struct virtq **out) {
  ...
  virtio_mmio_write32(dev->mmio_base, VIRTIO_MMIO_QUEUE_DESC_LOW, (uint32_t)(p_desc & 0xFFFFFFFFu));
  virtio_mmio_write32(dev->mmio_base, VIRTIO_MMIO_QUEUE_DESC_HIGH, (uint32_t)(p_desc >> 32));
  virtio_mmio_write32(dev->mmio_base, VIRTIO_MMIO_DRIVER_DESC_LOW, (uint32_t)(p_avail & 0xFFFFFFFFu));
  virtio_mmio_write32(dev->mmio_base, VIRTIO_MMIO_DRIVER_DESC_HIGH, (uint32_t)(p_avail >> 32));
  virtio_mmio_write32(dev->mmio_base, VIRTIO_MMIO_DEVICE_DESC_LOW, (uint32_t)(p_used & 0xFFFFFFFFu));
  virtio_mmio_write32(dev->mmio_base, VIRTIO_MMIO_DEVICE_DESC_HIGH, (uint32_t)(p_used >> 32));
  ...
  if (qidx < (uint16_t)(sizeof(dev->queues) / sizeof(dev->queues[0]))) {
    dev->queues[qidx] = q;
    if (qidx + 1 > dev->num_queues) dev->num_queues = (uint16_t)(qidx + 1);
  }

  // Allocate cookie array (separate page)
  virtq_internal_header_t *hdr = (virtq_internal_header_t *)kalloc(sizeof(virtq_internal_header_t));
  if (!hdr) return -5;
  hdr->cookies = (void **)kalloc(sizeof(void *) * size);
  if (!hdr->cookies) return -6;
  ...
  (void)desc_bytes; (void)avail_bytes; (void)used_bytes; // silence if unused
...
int virtq_poll_used(struct virtq *q, void **out_cookie, size_t *out_bytes) {
  ...
  q->last_used_idx++;
  if (out_cookie) *out_cookie = (void *)(uintptr_t)id; // head id as cookie
  if (out_bytes) *out_bytes = len;
  if (q->on_used) q->on_used(q, (void *)(uintptr_t)id, len, q->cb_user);
  return 0;
}
...
int virtq_submit(struct virtq *q, const struct iovec *out_sg, size_t out_cnt, const struct iovec *in_sg, size_t in_cnt, void *cookie) {
  ...
  for (size_t i = 0; i < out_cnt; i++) {
    ...
    q->desc[cur].flags = (i + 1 < out_cnt || in_cnt > 0) ? VRING_DESC_F_NEXT : 0;
    if (q->desc[cur].flags & VRING_DESC_F_NEXT) q->desc[cur].next = (uint16_t)(cur + 1);
    cur++;
  }
  ...
  for (size_t i = 0; i < in_cnt; i++) {
    ...
    q->desc[cur].flags = VRING_DESC_F_WRITE | ((i + 1 < in_cnt) ? VRING_DESC_F_NEXT : 0);
    if (q->desc[cur].flags & VRING_DESC_F_NEXT) q->desc[cur].next = (uint16_t)(cur + 1);
    cur++;
  }
...
void virtq_handle_irq_for_device(virtio_device_t *dev) {
  (void)dev;
  // In this minimal version, drivers call virtq_poll_used periodically or from ISR
}
