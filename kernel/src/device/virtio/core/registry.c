...
__attribute__((unused)) static const virtio_driver_t *find_driver(uint32_t device_id) {
...
void virtio_bus_init_from_dtb(void) {
  ...
  while ((cur = dtb_find_compatible(cur, "virtio,mmio")) != NULL) {
    ...
    // Allow multiple drivers to attach to same device id (e.g., input → kbd and mouse wrappers)
