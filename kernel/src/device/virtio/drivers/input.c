...
typedef struct { virtio_device_t *dev; virtio_input_dev_t *inp; } input_slot_t;
static input_slot_t g_inputs[MAX_INPUT_DEVS];

// Allow one primary callback (kbd or mouse) and an additional listener.
// This keeps compatibility while ensuring both kbd and mouse can receive events.
// Deprecated global extras; store in the instance now

static void event_used_cb(struct virtq *q, void *cookie, size_t bytes, void *user) {
  (void)bytes; (void)q;
  ...
  struct iovec in_sg = { .iov_base = &inp->events[idx], .iov_len = sizeof(struct virtio_input_event) };
...
static int input_init(virtio_input_dev_t *inp, uint16_t ring_size) {
  ...
  if (rc) return rc;
  virtio_input_status_pkt_t *pkt = (virtio_input_status_pkt_t *)kalloc(sizeof(virtio_input_status_pkt_t));
  pkt->select = 1; // STATUS
  pkt->reserved = 0;
  pkt->size = 1;
  ...
  // Post event buffers
  inp->events = (struct virtio_input_event *)kalloc(sizeof(struct virtio_input_event) * ring_size);
  inp->event_cap = ring_size;
  for (uint16_t i = 0; i < ring_size; i++) {
    struct iovec in_sg = { .iov_base = &inp->events[i], .iov_len = sizeof(struct virtio_input_event) };
  ...
  virtio_mmio_write32(inp->vdev->mmio_base, VIRTIO_MMIO_STATUS, s | VIRTIO_CONFIG_S_DRIVER_OK);
  return 0;
}
...
int virtio_input_probe(virtio_device_t *dev, uint16_t event_ring_size) {
  ...
  for (int i = 0; i < MAX_INPUT_DEVS; i++) {
    if (g_inputs[i].dev == dev) return 0;
  }
  // find free slot
  int free_idx = -1;
  for (int i = 0; i < MAX_INPUT_DEVS; i++) {
    if (g_inputs[i].dev == NULL) { free_idx = i; break; }
  }
  if (free_idx < 0) return -1;
  virtio_input_dev_t *inp = (virtio_input_dev_t *)kalloc(sizeof(virtio_input_dev_t));
  if (!inp) return -1;
...
void virtio_input_set_callback(virtio_input_dev_t *inp, virtio_input_event_cb cb, void *user) {
  inp->on_event = cb;
  inp->user = user;
}
...
virtio_input_dev_t *virtio_input_get(void) {
  ...
  for (int i = 0; i < MAX_INPUT_DEVS; i++) if (g_inputs[i].dev) return g_inputs[i].inp;
  return NULL;
}
virtio_input_dev_t *virtio_input_get_for(virtio_device_t *dev) {
  for (int i = 0; i < MAX_INPUT_DEVS; i++) if (g_inputs[i].dev == dev) return g_inputs[i].inp;
  return NULL;
}

int virtio_input_add_listener(virtio_input_dev_t *inp, virtio_input_event_cb cb, void *user) {
  inp->extra_cb = cb;
  inp->extra_user = user;
  return 0;
}
...
static void unified_on_event(const struct virtio_input_event *ev, void *user) {
  ...
  if (ev->type == 0x02) {
    if (ev->code == 0x00) inp->mouse_rel_x += (int32_t)ev->value; // REL_X
    if (ev->code == 0x01) inp->mouse_rel_y += (int32_t)ev->value; // REL_Y
    if (ev->code == 0x08) inp->mouse_wheel += (int32_t)ev->value; // REL_WHEEL
  ...
  if (ev->type == 0x01) {
    ...
    if (ev->code == 0x110 || ev->code == 0x111 || ev->code == 0x112) {
      uint8_t mask = 0;
      if (ev->code == 0x110) mask = 1u << 0;
      if (ev->code == 0x111) mask = 1u << 1;
      if (ev->code == 0x112) mask = 1u << 2;
      if (ev->value) inp->mouse_buttons |= mask; else inp->mouse_buttons &= (uint8_t)~mask;
      return;
    }

    if (ev->code == 0x3a) { // KEY_CAPSLOCK
      KEYPRESS_MODIFIER_SET(inp->kbd_modifiers, KEYPRESS_MODIFIER_CAPSLOCK, ev->value == 1);
      return;
    }

    // keypress stuff
    result_t rkp = make_keypress((uint8_t)ev->code, inp->kbd_modifiers, (keypress_type_t)ev->value);
    if (result_is_ok(rkp)) {
