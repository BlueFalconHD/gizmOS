...
const char *format_decimal_point_to_str(enum format_decimal_point format_decimal_point);
...
enum format_type format_type_from_str(const char *str) {
    if (strcmp(str, FORMAT_TYPE_INT_STR))        return FORMAT_TYPE_INT;
    else if (strcmp(str, FORMAT_TYPE_UINT_STR))  return FORMAT_TYPE_UINT;
    else if (strcmp(str, FORMAT_TYPE_HEX_STR))   return FORMAT_TYPE_HEX;
    else if (strcmp(str, FORMAT_TYPE_CHAR_STR))  return FORMAT_TYPE_CHAR;
    else if (strcmp(str, FORMAT_TYPE_STR_STR))   return FORMAT_TYPE_STR;
    else if (strcmp(str, FORMAT_TYPE_PTR_STR))   return FORMAT_TYPE_PTR;
    return FORMAT_TYPE_INVALID;
}

const char *format_type_to_str(enum format_type type) {
    switch (type) {
    case FORMAT_TYPE_INT:  return FORMAT_TYPE_INT_STR;
    case FORMAT_TYPE_UINT: return FORMAT_TYPE_UINT_STR;
    case FORMAT_TYPE_HEX:  return FORMAT_TYPE_HEX_STR;
    case FORMAT_TYPE_CHAR: return FORMAT_TYPE_CHAR_STR;
    case FORMAT_TYPE_STR:  return FORMAT_TYPE_STR_STR;
    case FORMAT_TYPE_PTR:  return FORMAT_TYPE_PTR_STR;
    default:                      return "INVALID_FORMAT_TYPE";
    }
}

enum format_case format_case_from_str(const char *str) {
    if (strcmp(str, FORMAT_CASE_LOWER_STR))       return FORMAT_CASE_LOWER;
    else if (strcmp(str, FORMAT_CASE_UPPER_STR))  return FORMAT_CASE_UPPER;
    return FORMAT_CASE_INVALID;
}

const char *format_case_to_str(enum format_case format_case) {
    switch (format_case) {
    case FORMAT_CASE_LOWER: return FORMAT_CASE_LOWER_STR;
    case FORMAT_CASE_UPPER: return FORMAT_CASE_UPPER_STR;
    default:                       return "INVALID_FORMAT_CASE";
    }
}

enum format_justify format_justify_from_str(const char *str) {
    if (strcmp(str, FORMAT_JUSTIFY_LEFT_STR))       return FORMAT_JUSTIFY_LEFT;
    else if (strcmp(str, FORMAT_JUSTIFY_RIGHT_STR)) return FORMAT_JUSTIFY_RIGHT;
    return FORMAT_JUSTIFY_INVALID;
}

const char *format_justify_to_str(enum format_justify format_justify) {
    switch (format_justify) {
    case FORMAT_JUSTIFY_LEFT:  return FORMAT_JUSTIFY_LEFT_STR;
    case FORMAT_JUSTIFY_RIGHT: return FORMAT_JUSTIFY_RIGHT_STR;
    default:                          return "INVALID_FORMAT_JUSTIFY";
    }
}

enum format_sign format_sign_from_str(const char *str) {
    if (strcmp(str, FORMAT_SIGN_AUTO_STR))   return FORMAT_SIGN_AUTO;
    else if (strcmp(str, FORMAT_SIGN_FORCE_STR))  return FORMAT_SIGN_FORCE;
    else if (strcmp(str, FORMAT_SIGN_SPACE_STR))  return FORMAT_SIGN_SPACE;
    return FORMAT_SIGN_INVALID;
}

const char *format_sign_to_str(enum format_sign format_sign) {
    switch (format_sign) {
    case FORMAT_SIGN_AUTO:   return FORMAT_SIGN_AUTO_STR;
    case FORMAT_SIGN_FORCE:  return FORMAT_SIGN_FORCE_STR;
    case FORMAT_SIGN_SPACE:  return FORMAT_SIGN_SPACE_STR;
    default:                        return "INVALID_FORMAT_SIGN";
    }
}

enum format_prefix format_prefix_from_str(const char *str) {
    if (strcmp(str, FORMAT_PREFIX_AUTO_STR))   return FORMAT_PREFIX_AUTO;
    else if (strcmp(str, FORMAT_PREFIX_NONE_STR)) return FORMAT_PREFIX_NONE;
    return FORMAT_PREFIX_INVALID;
}

const char *format_prefix_to_str(enum format_prefix format_prefix) {
    switch (format_prefix) {
    case FORMAT_PREFIX_AUTO: return FORMAT_PREFIX_AUTO_STR;
    case FORMAT_PREFIX_NONE: return FORMAT_PREFIX_NONE_STR;
    default:                        return "INVALID_FORMAT_PREFIX";
    }
}

enum format_decimal_point format_decimal_point_from_str(const char *str) {
    if (strcmp(str, FORMAT_DECIMAL_POINT_AUTO_STR))   return FORMAT_DECIMAL_POINT_AUTO;
    else if (strcmp(str, FORMAT_DECIMAL_POINT_FORCE_STR)) return FORMAT_DECIMAL_POINT_FORCE;
    return FORMAT_DECIMAL_POINT_INVALID;
}

const char *format_decimal_point_to_str(enum format_decimal_point format_decimal_point) {
    switch (format_decimal_point) {
    case FORMAT_DECIMAL_POINT_AUTO:  return FORMAT_DECIMAL_POINT_AUTO_STR;
    case FORMAT_DECIMAL_POINT_FORCE: return FORMAT_DECIMAL_POINT_FORCE_STR;
    default:                                return "INVALID_FORMAT_DECIMAL_POINT";
    }
}

enum format_left_pad format_left_pad_from_str(const char *str) {
    if (strcmp(str, FORMAT_LEFT_PAD_SPACE_STR)) return FORMAT_LEFT_PAD_SPACE;
    else if (strcmp(str, FORMAT_LEFT_PAD_ZERO_STR))  return FORMAT_LEFT_PAD_ZERO;
    return FORMAT_LEFT_PAD_INVALID;
}

const char *format_left_pad_to_str(enum format_left_pad format_left_pad) {
    switch (format_left_pad) {
    case FORMAT_LEFT_PAD_SPACE: return FORMAT_LEFT_PAD_SPACE_STR;
    case FORMAT_LEFT_PAD_ZERO:  return FORMAT_LEFT_PAD_ZERO_STR;
    default:                           return "INVALID_FORMAT_LEFT_PAD";
    }
}

char *format_dump(struct format *format, char *buf, size_t buf_len)
{
    // Make sure we don’t exceed buf_len; for simplicity, use a safe strcat or manual indexing.
...
void format_parse_single(struct format *format, const char *key, const char *val)
{
...
char *apply_format_generic(struct format *format, char *ret_buf, size_t ret_buf_len, void *val)
{
    // 1) Make a temp buffer for the raw numeric/string data (before justification).
    ...
    if (width < data_len) width = data_len;   // no negative padding
